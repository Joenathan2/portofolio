...
const Reveal = ({ children, delay = 0, className = "", instant = false }) => {
  ...
  return (
    <div
      ref={ref}
      className={`transition-all duration-1000 ease-[cubic-bezier(0.22,1,0.36,1)] ${isVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-10 scale-[0.98]'} ${className}`}
...
const App = () => {
  ...
  // State untuk menampung proyek yang sedang dibuka di modal
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      rafId = requestAnimationFrame(() => {
        ...
        if (bgOrb1Ref.current) bgOrb1Ref.current.style.transform = `translateY(${scrollY * 0.4}px)`;
        if (bgOrb2Ref.current) bgOrb2Ref.current.style.transform = `translateY(${scrollY * 0.2}px)`;
        if (heroTextRef.current) heroTextRef.current.style.transform = `translateY(${scrollY * 0.12}px)`;
      });
    };
  ...
  return (
    <div className="relative min-h-screen text-slate-100 selection:bg-cyan-500 selection:text-slate-900 overflow-x-hidden bg-[#020818]">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-40 transition-all duration-500 ${scrolled ? 'py-3 bg-[#020818]/85 backdrop-blur-xl border-b border-blue-900/40 shadow-lg shadow-black/20' : 'py-6 bg-transparent'}`}>
      ...
      <div
        ...
        style={{ background: `radial-gradient(550px circle at ${mousePos.x}px ${mousePos.y}px, rgba(34,211,238,0.06), transparent 80%)` }}
      />
      ...
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        ...
        <div ref={bgOrb1Ref} className="absolute inset-0 will-change-transform">
          <div className="absolute top-[-15%] left-[-15%] w-[75vw] h-[75vw] rounded-full mix-blend-screen opacity-50 animate-spin-slow"
               style={{ background: 'radial-gradient(circle, rgba(29,78,216,0.85) 0%, rgba(29,78,216,0) 65%)' }}/>
        </div>
        <div ref={bgOrb2Ref} className="absolute inset-0 will-change-transform">
          <div className="absolute top-[15%] right-[-25%] w-[65vw] h-[65vw] rounded-full mix-blend-screen opacity-40 animate-blob"
               style={{ background: 'radial-gradient(circle, rgba(6,182,212,0.75) 0%, rgba(6,182,212,0) 65%)' }}/>
        </div>
        <div className="absolute bottom-[-25%] left-[15%] w-[80vw] h-[80vw] rounded-full mix-blend-screen opacity-30 animate-pulse-slow"
             style={{ background: 'radial-gradient(circle, rgba(20,184,166,0.5) 0%, rgba(20,184,166,0) 60%)' }}/>
        <div className="absolute inset-0 opacity-[0.07] mix-blend-overlay bg-noise"></div>
        <div className="absolute inset-0 bg-grid-pattern opacity-100 pointer-events-none"></div>
      </div>

      <div className="relative z-10 w-full max-w-7xl mx-auto">

        {/* ── HERO (Disesuaikan dengan Profil CV & Fokus Web Utama) ── */}
        <section id="home" className="pt-28 md:pt-36 pb-16 md:pb-24 px-6 md:px-12 border-b border-blue-900/30">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center">

            <div ref={heroTextRef} className="lg:col-span-7 order-2 lg:order-1 text-center lg:text-left will-change-transform">
              <Reveal delay={80} instant={true}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-8 border-t border-blue-900/40 text-left">
                <Reveal delay={350} className="sm:col-span-1" instant={true}>
                ...
                <Reveal delay={450} className="sm:col-span-2" instant={true}>
                  ...
                  <div className="flex flex-wrap gap-3">
                    <a href="#projects" className="bg-cyan-500 text-[#020818] px-6 py-3 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-cyan-400 hover:scale-105 transition-all duration-300 shadow-[0_0_25px_rgba(34,211,238,0.25)]" style={{ fontFamily: "'Syne', sans-serif" }}>
                      Lihat Proyek
                    </a>
                    <a href="#contact" className="border border-cyan-500/40 text-cyan-200 px-6 py-3 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-cyan-500/10 hover:border-cyan-400/70 transition-all duration-300" style={{ fontFamily: "'Syne', sans-serif" }}>
                      Hubungi Saya
                    </a>
                  </div>
                </Reveal>
              ...
              <Reveal delay={280} instant={true}>
                <div className="relative w-48 h-64 sm:w-60 sm:h-80 md:w-80 md:h-[27rem] lg:w-[22rem] lg:h-[30rem] rounded-2xl p-[3px] lg:-rotate-2 hover:rotate-0 transition-all duration-700 group"
                  ...
                  <div className="w-full h-full rounded-xl overflow-hidden bg-blue-950/40">
                    <img
                      src="./src/file_0000000095f8720ba83999cd3b5712cd.png"
        ...
        {/* ── PROJECTS (Ditingkatkan dengan Fitur Modal Ekspansi Detail & Animasi Klik) ── */}
        ...
        {/* ── MODAL POPUP DETAIL PROYEK (Dengan Peningkatan Animasi Masuk) ── */}
        {selectedProject && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-10">
            {/* Backdrop Blur Overlay dengan Animasi Fade-In */}
            <div 
              className="absolute inset-0 bg-slate-950/85 backdrop-blur-lg transition-opacity duration-300 animate-fade-in"
              onClick={() => setSelectedProject(null)}
            />
            
            {/* Modal Body dengan Efek Animasi Membesar Spring-Scale */}
            <div className="relative bg-[#040d21] border border-blue-900/60 rounded-3xl w-full max-w-4xl max-h-[85vh] overflow-y-auto shadow-2xl shadow-cyan-950/40 z-10 animate-scale-in">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12">
                ...
                {/* Bagian Deskripsi Detail (Kanan/Bawah) */}
        ...
        <footer id="contact" className="px-6 md:px-12 py-20 md:py-32 relative scroll-mt-20">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-end">
            ...
            <Reveal delay={300} className="flex flex-col justify-end lg:text-right">
      ...
      <style>{`
        ...
        * { font-family: 'DM Sans', sans-serif; }
        ...
        @keyframes spin-slow {
          ...
          50%  { transform: rotate(180deg) translate(3vw, -5vh) scale(1.1); }
          100% { transform: rotate(360deg) translate(0px, 0px) scale(1); }
        }
        @keyframes blob {
          0%   { transform: translate(0vw, 0vh) scale(1); }
          33%  { transform: translate(-4vw, 5vh) scale(1.2); }
          66%  { transform: translate(3vw, -3vh) scale(0.9); }
          100% { transform: translate(0vw, 0vh) scale(1); }
        }
        @keyframes pulse-slow {
          0%   { transform: scale(1) translate(0vw, 0vh); opacity: 0.25; }
          50%  { transform: scale(1.35) translate(-2vw, -2vh); opacity: 0.5; }
          100% { transform: scale(1) translate(0vw, 0vh); opacity: 0.25; }
        }
        @keyframes fadeIn {
        ...
        .animate-spin-slow  { animation: spin-slow  28s infinite linear; }
        .animate-blob       { animation: blob       22s infinite alternate ease-in-out; }
        .animate-pulse-slow { animation: pulse-slow 18s infinite alternate ease-in-out; }
